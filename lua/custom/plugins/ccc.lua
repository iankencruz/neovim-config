return {
  ...
  event = 'FileType',
  keys = {
    { '<Leader>cpp', '<cmd>CccPick<CR>', desc = 'Color-picker' },
    { '<Leader>cpt', '<cmd>CccHighlighterToggle<CR>', desc = 'Toggle Color Highleter' },
    { '<Leader>cpc', '<cmd>CccConvert<CR>', desc = 'Convert to next output' },
  },
  opts = function()
    ...
    return {
      ...
      -- Top-level inputs for the picker itself
      inputs = {
        ccc.input.hsl,
        ccc.input.rgb, -- Add this if you want RGB available too
        ccc.input.oklch,
        RgbHslCmykInput,
      },
      outputs = {
      ...
      convert = {
        ...
        { ccc.picker.css_oklch, ccc.output.css_rgb },
